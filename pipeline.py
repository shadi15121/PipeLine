...
print("\n--- Kendall Correlation ---")
print(numeric_df.corr(method='kendall'))
...
plt.figure(figsize=(8, 6))
sns.heatmap(numeric_df.corr(method='spearman'), annot=True, fmt=".2f", cmap="viridis")
plt.title("Spearman Correlation Heatmap")
plt.show()

# Kendall Correlation Heatmap
plt.figure(figsize=(8, 6))
sns.heatmap(numeric_df.corr(method='kendall'), annot=True, fmt=".2f", cmap="magma")
...
print("\n=== CROSSTABS BETWEEN ALL CATEGORICAL FEATURES ===")

# Step 1: Copy the original DataFrame
df_crosstab = df.copy()

# Step 2: Create binned versions of numeric features (so we can treat them as categories)
bins = 5  # Adjust the number of bins if needed
df_crosstab['step_bin'] = pd.qcut(df['step'], q=bins, duplicates='drop')
df_crosstab['amount_bin'] = pd.qcut(df['amount'], q=bins, duplicates='drop')
df_crosstab['oldbalanceOrg_bin'] = pd.qcut(df['oldbalanceOrg'], q=bins, duplicates='drop')
df_crosstab['newbalanceOrig_bin'] = pd.qcut(df['newbalanceOrig'], q=bins, duplicates='drop')
df_crosstab['oldbalanceDest_bin'] = pd.qcut(df['oldbalanceDest'], q=bins, duplicates='drop')
df_crosstab['newbalanceDest_bin'] = pd.qcut(df['newbalanceDest'], q=bins, duplicates='drop')

# Step 3: Identify categorical/binned features
categorical_cols = [
    'type', 'isFraud', 'isFlaggedFraud',
    'step_bin', 'amount_bin', 'oldbalanceOrg_bin',
    'newbalanceOrig_bin', 'oldbalanceDest_bin', 'newbalanceDest_bin'
]
import seaborn as sns
import matplotlib.pyplot as plt
# Step 4: Loop through all unique pairs and create crosstabs
for col1, col2 in combinations(categorical_cols, 2):
    print(f"\n--- Crosstab between '{col1}' and '{col2}' ---")
    ct = pd.crosstab(df_crosstab[col1], df_crosstab[col2], margins=True)
    print(ct)
    import seaborn as sns
    import matplotlib.pyplot as plt

    # Inside the loop (after printing the crosstab):
    plt.figure(figsize=(10, 6))
    sns.heatmap(ct.iloc[:-1, :-1], annot=True, fmt='g', cmap='Blues')  # Exclude margins
...
print("\n=== Association Analysis: Chi-Squared Test ===")

from scipy.stats import chi2_contingency

# Create a contingency table between transaction type and fraud
...
if p < 0.05:
    print("Result: Significant association between transaction type and fraud.")
else:
    print("Result: No significant association found.")

# Bar plot for transaction type vs fraud
plt.figure(figsize=(8, 5))
pd.crosstab(df['type'], df['isFraud']).plot(kind='bar', stacked=True, colormap='Set2')
